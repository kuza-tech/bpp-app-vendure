...
import { CreateSellerResolver } from './api/cs.resolver';
import { createSellerShippingEligibilityChecker } from './config/cs-shipping-eligibility-checker';
import { CreateSellerService } from './service/cs.service';
import { createSellerPaymentMethodHandler } from './config/cs-payment-handler';
...
@VendurePlugin({
    ...
    configuration: config => {
        ...
        config.paymentOptions.paymentMethodHandlers.push(createSellerPaymentMethodHandler);
        config.shippingOptions.shippingEligibilityCheckers.push(createSellerShippingEligibilityChecker);
        return config;
    },
    shopApiExtensions: {
        schema: shopApiExtensions,
        resolvers: [CreateSellerResolver],
    },
    providers: [
        CreateSellerService,
    ],
})
export class CreateSellerPlugin implements OnApplicationBootstrap {

    constructor(
    ) {}

    static init() {
        return CreateSellerPlugin;
    }
